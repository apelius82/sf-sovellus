...
class PreviewImageGenerator
{
    ...
    private function contentFitsOnSingleCard(
        ...
        $contentWidth = self::SPLIT_WIDTH - (self::SPLIT_VIEW_PADDING_SMALL * 2);
    ...
    private function renderCard(array $flashData, string $templatePath, string $outputPath, int $cardNumber): void
    {
        ...
        try {
            ...
            if ($cardNumber === 2) {
                ...
                $contentTopY = self::SPLIT_Y + self::SPLIT_VIEW_HEADER_HEIGHT + self::SPLIT_VIEW_TOP_PADDING + $contentFontSize;
                $metaTopY = self::META_Y - self::SPLIT_VIEW_META_BUFFER;
                $availableHeight = max(0, ($metaTopY - 16) - $contentTopY);

                [$rootCard1, $rootCard2] = $this->splitBulletContentForHeight($rootCauses, $contentWidth, $contentFontSize, $availableHeight);
                [$actionsCard1, $actionsCard2] = $this->splitBulletContentForHeight($actions, $contentWidth, $contentFontSize, $availableHeight);

                $this->renderCard2($imagick, $title, $rootCard2, $actionsCard2, $labels, $flashData);
    ...
    private function renderCard2(
        ...
        $titleLines = $this->wrapTextWithParagraphs(
            $title,
            (int) floor(($descBoxW - 40) / max(1, ($titleFontSize * 0.55)))
        );
    ...
    private function renderInvestigationSplitView(
        ...
        $maxContentLines = max(1, (int) floor($availableHeight / max(1, ($contentFontSize * self::SPLIT_VIEW_LINE_HEIGHT_MULTIPLIER))));
