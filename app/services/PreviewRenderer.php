...
class PreviewRenderer
{
    ...
    private function renderGreenCardSingle($image, array $data): void
    {
        ...
        $descText = trim((string) ($data['description'] ?? ''));
        ...
        $rootText = $this->formatBulletPoints((string) ($data['root_causes'] ?? ''));
        $actionsText = $this->formatBulletPoints((string) ($data['actions'] ?? ''));
        ...
        $headerHeight = 45;
        $contentPadding = 15;
        $contentTopY = $y + $headerHeight + $contentPadding;
        $contentBottomY = $metaY - 20;
        $availableContentHeight = max(0, $contentBottomY - $contentTopY);

        [$rootCard1, $rootCard2] = $this->splitBulletContentForHeight($rootText, $colWidth - 30, $contentFontSize, $availableContentHeight);
        [$actionsCard1, $actionsCard2] = $this->splitBulletContentForHeight($actionsText, $colWidth - 30, $contentFontSize, $availableContentHeight);

        $rootHeight = $this->measureBulletTextHeight($rootCard1, $colWidth - 30, $contentFontSize);
        $actionsHeight = $this->measureBulletTextHeight($actionsCard1, $colWidth - 30, $contentFontSize);
        $boxContentHeight = max($rootHeight, $actionsHeight, 50);

        $boxHeight = max(120, $headerHeight + 20 + $boxContentHeight);
    ...
    private function renderGreenCard2($image, array $data): void
    {
        ...
        $card1AvailableHeight = max(0, (self::HEIGHT - self::MARGIN_BOTTOM - self::META_BOX_HEIGHT - 16) - (self::DESC_START_Y + 45 + 15));
        [$rootCard1, $rootCard2] = $this->splitBulletContentForHeight($rootText, self::COLUMN_WIDTH - 30, $contentFontSize, $card1AvailableHeight);
        [$actionsCard1, $actionsCard2] = $this->splitBulletContentForHeight($actionsText, self::COLUMN_WIDTH - 30, $contentFontSize, $card1AvailableHeight);

        $rootCausesLabel = $this->getTerm('root_causes_label', $lang);
        $actionsLabel = $this->getTerm('actions_label', $lang);

        $this->renderLabeledBox($image, $colX1, $y, self::CARD2_COL_WIDTH, self::CARD2_COL_HEIGHT, $rootCausesLabel, $rootCard2, 18, $contentFontSize);
        $this->renderLabeledBox($image, $colX2, $y, self::CARD2_COL_WIDTH, self::CARD2_COL_HEIGHT, $actionsLabel, $actionsCard2, 18, $contentFontSize);
    }
    ...
    private function renderText($image, int $x, int $y, int $maxWidth, int $maxHeight, string $text, string $color, int $fontSize, string $fontWeight = 'Regular'): void
{
    ...
    private function contentFitsOnSingleCard(string $title, string $description, string $rootCauses, string $actions, array $sizes): bool
    {
        ...
        $rootCauses = $this->formatBulletPoints(trim((string) $rootCauses));
        $actions = $this->formatBulletPoints(trim((string) $actions));

        $descLines = $this->estimateLinesWithFontSize($description, self::CARD1_DESC_WIDTH, (int) $sizes['description'], 0.55, 18);
        $descHeight = $descLines * ((int) $sizes['description'] * 1.35);
        ...
        $contentWidth = self::COLUMN_WIDTH - 30;

        $headerHeight = 45;
        $contentPadding = 15;
        $metaTopY = self::HEIGHT - self::MARGIN_BOTTOM - self::META_BOX_HEIGHT;
        $contentTopY = self::DESC_START_Y + $descHeight + 30 + $headerHeight + $contentPadding;
        $safetyMargin = 16;
        $availableHeight = max(0, ($metaTopY - $safetyMargin) - $contentTopY);

        [$rootCard1, $rootCard2] = $this->splitBulletContentForHeight($rootCauses, $contentWidth, $contentFontSize, $availableHeight);
        [$actionsCard1, $actionsCard2] = $this->splitBulletContentForHeight($actions, $contentWidth, $contentFontSize, $availableHeight);

        return $rootCard2 === '' && $actionsCard2 === '';
    }
    ...
    private function estimateLinesWithFontSize(
        ...
        foreach (explode("\n", $text) as $paragraph) {
            ...
            foreach ($words as $word) {
                ...
                if (mb_strlen($testLine) > $charsPerLine && $currentLine !== '') {
                    $lines++;
                    $currentLine = $word;
                } else {
                    $currentLine = $testLine;
    ...
    /**
     * Measure bullet content height using the same idea as the rendered layout:
     * bullet indent + wrapped text + spacing between bullet items.
     */
    ...
    /**
     * Split bullet text into items while preserving plain text lines as items too.
     */
    ...
    /**
     * Join bullet items back to text.
     */
    ...
    /**
     * Split content into first-card and second-card parts based on available height.
     */
    ...
    public function needsSecondCard(array $data): bool
    {
        ...
        $sizes = $this->getFontSizes($data);

        return !$this->contentFitsOnSingleCard(
            trim((string) ($data['short_text'] ?? '')),
            trim((string) ($data['description'] ?? '')),
            trim((string) ($data['root_causes'] ?? '')),
            trim((string) ($data['actions'] ?? '')),
            $sizes
        );
    }
