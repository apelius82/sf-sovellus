...
<?php if ($user && (int)$user['role_id'] === 1): ?>
    ...
    <a href="<?= htmlspecialchars($base) ?>/index.php?page=playlist_manager"
       class="sf-nav-link <?= $currentPage === 'playlist_manager' ? 'sf-nav-active' : '' ?>"
       data-tooltip="Infonäytöt">
        <img src="<?= htmlspecialchars($base) ?>/assets/img/icons/display.svg"
             alt=""
             class="sf-nav-link-icon"
             aria-hidden="true">
    </a>
...
<?php endif; ?>
        ...
        <div class="sf-nav-right">
            ...
            <?php if ($user && (int)$user['role_id'] === 1): ?>
                <!-- Mobile Display Playlists Icon (Admin Only) -->
                <a href="<?= htmlspecialchars($base) ?>/index.php?page=playlist_manager"
                   class="sf-nav-settings-mobile <?= $currentPage === 'playlist_manager' ? 'sf-nav-active' : '' ?>"
                   aria-label="<?= htmlspecialchars(sf_term('nav_display_playlists', $uiLang) ?? 'Infonäytöt', ENT_QUOTES, 'UTF-8') ?>">
                    <img src="<?= htmlspecialchars($base) ?>/assets/img/icons/display.svg"
                         alt=""
                         class="sf-nav-settings-icon"
                         aria-hidden="true">
                </a>

                <!-- Mobile Settings Icon (Admin Only) -->
                <a href="<?= htmlspecialchars($base) ?>/index.php?page=settings"
                   class="sf-nav-settings-mobile <?= $currentPage === 'settings' ? 'sf-nav-active' : '' ?>"
                   aria-label="<?= htmlspecialchars(sf_term('settings_heading', $uiLang) ?? 'Asetukset', ENT_QUOTES, 'UTF-8') ?>">
                    <img src="<?= htmlspecialchars($base) ?>/assets/img/icons/settings.svg"
                         alt=""
                         class="sf-nav-settings-icon"
                         aria-hidden="true">
